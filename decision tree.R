...
library(rpart)
#Training the model
classifier5 = rpart(formula = Loan_Status ~ .,
                    data = train_set)

#pruning the tree
rules_pr<-prune(classifier5, cp=classifier5$cptable[which.min(classifier5$cptable[,"xerror"]),"CP"])

length(rules_pr$frame$var[rules_pr$frame$var=="<leaf>"])

library(rpart.plot)
#we can find the resulting decision tree based on rules
prp(rules_pr, type=1, extra=1, split.font=1, varlen=-10)

classifier5<-rules_pr
#predicting the value for test data
y_pred5 = predict(classifier5, newdata = test_set[-12], type = 'prob')

test_set$propensity<- y_pred5[,1]

#Arranging the predict_value in descending order
test_set<-test_set %>% arrange(desc(propensity))

test_set$Predicted_loan_status<-as.factor(ifelse(test_set$propensity >0,'Y','N'))

#Computing the confusion matrix for cutoff 0 with success class as 1.
library(caret)

confusion_0 <- confusionMatrix(test_set$Predicted_loan_status,test_set$Loan_Status, positive = "Y")
confusion_0
...
test_set$Predicted_loan_status<-as.factor(ifelse(test_set$propensity >0.25,'Y','N'))


confusion_25 <- confusionMatrix(test_set$Predicted_loan_status,test_set$Loan_Status, positive = "Y")
confusion_25
...
test_set$Predicted_loan_status<-as.factor(ifelse(test_set$propensity >0.5,'Y','N'))


confusion_50 <- confusionMatrix(test_set$Predicted_loan_status,test_set$Loan_Status, positive = 'Y')
confusion_50
...
test_set$Predicted_loan_status<-as.factor(ifelse(test_set$propensity >0.75,'Y','N'))

confusion_75 <- confusionMatrix(test_set$Predicted_loan_status,test_set$Loan_Status, positive = "Y")
confusion_75
...
test_set$Predicted_loan_status<-as.factor(ifelse(test_set$propensity >1,'Y','N'))

confusion_100 <- confusionMatrix(test_set$Predicted_loan_status,test_set$Loan_Status, positive = "Y")
confusion_100
...
plot.roc(r2, main="ROC Curve for Decision tree (AUC=0.72)")
auc(r2)
#The area under the curve is 0.72, which implies the model is a perfect classifier.
library(gains)

gain1<-gains(as.numeric(factor(test_set$Loan_Status, levels = c('N','Y'), labels = c(0,1))),as.numeric(test_set$propensity))
barplot(gain1$mean.resp/mean(as.numeric(factor(test_set$Loan_Status, levels = c('N','Y'), labels = c(0,1)))),names.arg = gain1$depth, xlab = "Percentile", ylab="Mean Response", main="Decision Tree Decile-wise lift chart ")
