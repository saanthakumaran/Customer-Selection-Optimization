...
#changing variable from char to categorical
...
loan_prediction$Loan_Amount_Term<-as.factor(loan_prediction$Loan_Amount_Term)
...
#Training the model
classifier6 = randomForest(formula=Loan_Status~., data=train_set, ntree=500)

#predicting the value for test data
y_pred5 <- predict(classifier6, newdata = test_set[-12], type = 'prob')

varImpPlot(classifier6)
...
plot.roc(r2, main="ROC Curve of Random Forest (AUC=0.77)")
auc(r2)
#The area under the curve is 0.78, which implies the model is a perfect classifier.
library(gains)

gain1<-gains(as.numeric(factor(test_set$Loan_Status, levels = c('N','Y'), labels = c(0,1))),as.numeric(test_set$propensity))
barplot(gain1$mean.resp/mean(as.numeric(factor(test_set$Loan_Status, levels = c('N','Y'), labels = c(0,1)))),names.arg = gain1$depth, xlab = "Percentile", ylab="Mean Response", main="Random Forest Decile-wise lift chart")

#From decile chart we could consider first 7 decile to get a loan_status data.
